...
//  parser
//
//  Created by Grant Janavs on 10/5/20.
...
//  Reader
//
//  Created by Grant Janavs on 9/24/20.
//  Copyright © 2020 Grant Janavs. All rights reserved.
//
...
{
	int lengthvallist = 0;
	while(vl != 0) //end of valuelist
	{
		vl=vl->tail;
		lengthvallist++;
	}
	return lengthvallist;
}/* lengthVL */
...
int lengthNL ( NAMELIST nl)
{
  ...
  while(nl != 0) //end of namelist
  {
	 nl = nl->tail;
	  lengthnamelist++;
  }
  return lengthnamelist;
}/* lengthNL */
...
{
	NUMBER n, n1, n2;
	n1 =vl->head;  //print something value list should have arg being printed
	if(op==11){ //print
		cout << n1 <<endl;
		return n1;
	}
...
{
	if(el==0){
		return 0; //this is putting null at end
	}
...
NUMBER applyCtrlOp ( int op,  EXPLIST args, ENV rho )
{
	...
	if(op == 0){   //(if e1 e2 e3 )
		...
		if(op==2){ //set
			//(1) evaluate e1--->n
			//check if x is in local if so set x to n return n
			//check if x is in global if so set x to n and return n
			//add x to global ENV (using bindvar)
			n = eval(args->tail->head,rho);
			NAME x = args->head->varble; //have x now
			if(isBound(x, rho)){
				assign(x, n, rho);
			}
		...
		if(op==3){ //(begin e1, e2, ....en)
			while(args!= 0){  //args not = tail
				n=eval(args->head, rho);
				args= args->tail;
			}
			return n;
		}
	return 0; //crash if not in any of these
